...
# Load the JSON file
with open('t5NEWSQA_test_newsqa_with_generated_answers_final.json', 'r') as f:
    data = json.load(f)
...
for item in tqdm(data['data'][:150], desc="Processing", dynamic_ncols=True):
    text = item['text']
...
# Calculate averages
total_questions = sum(len(item['questions']) for item in data['data'][:150])
